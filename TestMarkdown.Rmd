...
```{r Animation, results='hide', error=FALSE, message=FALSE, warning=FALSE}
library(gganimate)
library(gifski)
...
Before we begin adding features, we first create a time-series panel, which creates a unique combination of each block to the hour and day. This creates create a “panel” dataset where each time period is represented by a row, whether a parking session took place then or not. If a block did not have any parking sessions on it a given hour, a zero in that spot in the panel. If we had more processing power, this could be done at the fifteen or even ten minute intervals.
