...
Begin by loading required libraries and functions. 
...
Load data for San Francisco geometry including county lines and neighborhood outlines. Load San Francisco census data including median income, median age, percentage of white population,  travel time to work, number of commuters, means of public trasit, and total public transit usage. Load San Francisco census tract geometry. 
...
Load San Francisco parking meter usage data for one week - from April 30th to May 4th of 2018. Select only relevant subset of data, filter for on street metered parking only, and ceate time intervals for sesssion end time. Time intervals are created using lubridate package.
...
Add Weather data. A week with little weather change was selected to maximize consistency in parking trends between train days and test days. The following function downloads weather data from San Francisco Airport (SFO) between April 30th and May 4th 2018. 
...
The following plot illustrates the number of parking sessions per hour during the week. The consistency in sessions per day allows the model to predict parking sessions on Thursday and Friday based on the training data from Monday - Wednesday.
...
ggplot() +
  ...
  geom_sf(data=st_as_sfc(st_bbox(sfbox)), colour="#df4d41", fill=NA)
...
```{r testing data, results='hide', error=FALSE, message=FALSE, warning=FALSE}
...
netsample <- sample_n(ride.panel, 100)%>%
  na.omit()
...
week_predictions %>%
  ...
  ggplot(aes(week, MAE)) + 
  geom_bar(aes(fill = Regression), color = "#df4d41", position = "dodge", stat="identity") +
...
week_predictions %>% 
  ...
  geom_line(size = 1.1, color = "#df4d41") + 
